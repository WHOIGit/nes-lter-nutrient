...
filtered_cruises_aligned <- filtered_cruises %>% select(all_of(common_columns))
missing_columns <- setdiff(names(nutrients), names(filtered_cruises_aligned))
filtered_cruises_aligned[missing_columns] <- NA
filtered_cruises_aligned <- filtered_cruises_aligned %>% select(names(nutrients))
nutrients_updated <- bind_rows(nutrients, filtered_cruises_aligned)
...
nutrients <- nutrients %>%
  mutate(
    ...
    cruise_ID = ifelse(is.na(cruise_ID), "", cruise_ID)
  )
