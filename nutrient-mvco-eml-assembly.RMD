...
nutrients_input <- read_excel('MVCO_Nut_reps_copy20260629.xlsx') 
...
nutrients <- nutrients %>%
  ...
  relocate(cruise, .before = datetime) 
...
filtered_nut <- filtered_nut %>%
  mutate(ship = case_when(
    grepl("^AR", cruise) ~ "Neil Armstrong",
    grepl("^AT", cruise) ~ "Atlantis",
    grepl("^EN", cruise) ~ "Endeavor",
    grepl("^HRS", cruise) ~ "Hugh R. Sharp",
    grepl("^AE", cruise) ~ "Atlantic Explorer",
...
# Set values below detection to zero
# Transect cruises have values below detection set to the detection limit. Change these to zero to match the rest of this data set
# nitrate 0.04, ammonium 0.01, phosphate 0.009, silicate 0.03
filtered_nut <- filtered_nut |>
  mutate(nitrate = case_when(nitrate == 0.04 ~ 0, TRUE ~ nitrate)) |>
  mutate(ammonium = case_when(ammonium == 0.01 ~ 0, TRUE ~ ammonium))|>   
  mutate(phosphate = case_when(phosphate == 0.009 ~ 0, TRUE ~ phosphate)) |>           
  mutate(silicate = case_when(silicate == 0.03 ~ 0, TRUE ~ silicate))
...
ggplot(data = nutrients, aes(x = date_time_utc, y = nitrate_nitrite)) + geom_point() +
  xlab("date") + ylab("nitrate concentration (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("timeseries_point_nitrate.png")

ggplot(data = nutrients, aes(x = date_time_utc, y = ammonium)) + geom_point() +
  xlab("date") + ylab("ammonium concentration (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("ammonium_point_nitrate.png")

ggplot(data = nutrients, aes(x = date_time_utc, y = phosphate, color = iode_quality_flag_phosphate)) + geom_point() +
  xlab("date") + ylab("phosphate (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("timeseries_point_phosphate.png")

ggplot(data = nutrients, aes(x = date_time_utc, y = silicate)) + geom_point() +
  xlab("date") + ylab("silicate concentration (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("timeseries_point_silicate.png")

# line plots
ggplot(data = nutrients, aes(x = date_time_utc, y = nitrate_nitrite)) + geom_line() +
  xlab("date") + ylab("nitrate concentration (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("timeseries_nitrate.png")

ggplot(data = nutrients, aes(x = date_time_utc, y = ammonium)) + geom_line() +
  xlab("date") + ylab("ammonium concentration (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("ammonium_nitrate.png")

ggplot(data = nutrients, aes(x = date_time_utc, y = phosphate, color = iode_quality_flag_phosphate)) + geom_line() +
  xlab("date") + ylab("phosphate (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("timeseries_phosphate.png")

ggplot(data = nutrients, aes(x = date_time_utc, y = silicate)) + geom_line() +
  xlab("date") + ylab("silicate concentration (µmol/L)") +
  scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", date_labels = "%Y") +
  theme_minimal()
ggsave("timeseries_silicate.png")
...
# # to use this method, update to use authentication
# url <- "https://pasta.lternet.edu/package/data/eml/knb-lter-nes/1/5/077bb6d32f84c773852b678f1fe52e57"
# httr::GET(url, write_disk(tf <- tempfile(fileext = ".csv")))
# tf
# nutr_pubd <- read_csv(tf, TRUE, show_col_types = FALSE)

nutr_pubd <- read_csv("nes-lter-nutrient-mvco_v5.csv", TRUE, show_col_types = FALSE)
