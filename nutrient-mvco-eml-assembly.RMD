...
map_locs(nutrients_long, colorvar = "depth", region = "transect")

# investigate distribution of low-latitude samples
hist(nutrients_long$latitude, plot = FALSE)

# filter samples into northern and southern sections using cutoff of 41.30 deg N suggested by histogram data
southern_samples <- nutrients_long %>% filter(latitude < 41.30 )
northern_samples <- nutrients_long %>% filter(latitude > 41.30 )
...
nutrients_long <- rename(nutrients_long, nitrate_nitrite = nitrate) %>%
  relocate(phosphate, .before = silicate)

# write csv file for package
write.csv(nutrients_long, "nes-lter-nutrient-mvco.csv", row.names = FALSE)
...
make_eml(here(),
         dataset.title='Dissolved inorganic nutrients from NES-LTER cruises to the Marthas Vineyard Coastal Observatory (MVCO), including 4 macro-nutrients from water column bottle samples, ongoing since 2006', 
         data.table='nes-lter-nutrient-mvco.csv',
         data.table.description='Dissolved inorganic nutrients from water column bottle samples taken on NES-LTER Transect cruises to the Marthas Vineyard Coastal Observatory',
         data.table.name = 'nes-lter-nutrient-mvco',
         temporal.coverage = temporal_coverage(data_table$date),
         geographic.description = "NES-LTER Transect",  #should be MVCO?
